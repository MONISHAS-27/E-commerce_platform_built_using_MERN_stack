...
app.post('/signup', async (req, res) => {
   ...
   const token = jwt.sign(data, process.env.JWT_SECRET); // Use the JWT_SECRET from .env
...
const fetchUser   = async (req, res, next) => {
   ...
   if (!token) {
      ...
      try {
         const data = jwt.verify(token, process.env.JWT_SECRET); // Use the JWT_SECRET from .env
         req.user = data.user;
         next();
      } catch (error) {
         res.status(401).send({
